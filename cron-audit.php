...
$last90 =  date('Y-m-d', strtotime("-1 days")); 
	...
	$sqlROW['fb_id'] = '107704242648697';
	$page_id = '253509791497350';
	
	$query_pg = "SELECT pg_token FROM pages WHERE pg_id='".$page_id."'";
$result_pg = mysqli_query($conn, $query_pg);
$row_pg = mysqli_fetch_assoc($result_pg);

$pg_access_token = $row_pg['pg_token']; 
...
$lp_urls = array();
function ObjResults($url, $conn, $accId, $pgId, $obj_arr) { 
	...
	if(isset($fb_response->paging->next)) {
		ObjResults($url, $conn, $accId, $pgId, $obj_arr);
	} else {
		$res = '';
		...
		mysqli_query($conn, "UPDATE audit_data SET obj_cost_result='".$res."' WHERE acc_id=".$accId."") or die(mysqli_error());
		//echo $res;
	}
}

function AdSet($url, $conn, $accId) { 
	...
	foreach ($fb_response->data as $key => $res) {
			...
			$cirRes = mysqli_query($conn, "select * from audit_adset WHERE adset_id='".$response->id."'");						
			
			//if(mysqli_num_rows($cirRes)==0) {
					$cirSql = "INSERT INTO audit_adset (adset_id, adset_name, bid_strategy, is_dynamic_creative, adset_type, age_group, interests, geo_locations, campaign_id, daily_budget, lifetime_budget, work_employers, work_positions, pacing_type, expansion, placement, updated) VALUES ('".mysqli_real_escape_string($conn, $res->id)."', '".mysqli_real_escape_string($conn, $res->name)."', '".mysqli_real_escape_string($conn, $bid)."', '".mysqli_real_escape_string($conn, $dynamic)."', '".mysqli_real_escape_string($conn, $res->adcreatives->data[0]->object_type)."', '".mysqli_real_escape_string($conn, $res->targeting->age_min.'-'.$res->targeting->age_max)."', '".mysqli_real_escape_string($conn, $intr_name)."', '".mysqli_real_escape_string($conn, $loc_name)."', '".mysqli_real_escape_string($conn, $res->campaign_id)."', '".mysqli_real_escape_string($conn, $res->daily_budget)."', '".mysqli_real_escape_string($conn, $res->lifetime_budget)."', '".mysqli_real_escape_string($conn, $w_emp_name)."', '".mysqli_real_escape_string($conn, $w_pos_name)."', '".mysqli_real_escape_string($conn, $pacing_type)."', '".mysqli_real_escape_string($conn, $expansion)."',  '".mysqli_real_escape_string($conn, $placement)."', now());"; 
					mysqli_query($conn, $cirSql) or die(mysqli_error()); 
			//	} 
	}  
	if(isset($fb_response->paging->next)) {
		AdSet($fb_response->paging->next, $conn, $accId);
	} else {
		$bid_strategy = $is_dynamic_creative = $adset_type = $interests = $geo_locations = $interests = $work_positions = $work_positions = array();
		$daily_budget = $lifetime_budget = $day_part = '';
		$placement = $age_group = array();
		$sql_1 = mysqli_query($conn, "SELECT age_group, count(tbl_id) as tot FROM audit_adset group by age_group");
		while($row1 = mysqli_fetch_array($sql_1)) { $age_group[]  = $row1['age_group'].' : '.$row1['tot']; }
		
		$sql_2 = mysqli_query($conn, "SELECT bid_strategy, count(tbl_id) as tot FROM audit_adset group by bid_strategy");
		while($row2 = mysqli_fetch_array($sql_2)) { $bid_strategy[]  = $row2['bid_strategy'].' : '.$row2['tot']; }
		
		$sql_3 = mysqli_query($conn, "SELECT is_dynamic_creative, count(tbl_id) as tot FROM audit_adset group by is_dynamic_creative");
		while($row3 = mysqli_fetch_array($sql_3)) { $is_dynamic_creative[]  = $row3['is_dynamic_creative'].' : '.$row3['tot']; }
		
		$sql_4 = mysqli_query($conn, "SELECT adset_type, count(tbl_id) as tot FROM audit_adset group by adset_type");
		while($row4 = mysqli_fetch_array($sql_4)) { $adset_type[]  = $row4['adset_type'].' : '.$row4['tot']; }
		
		$sql_5 = mysqli_query($conn, "SELECT geo_locations, count(tbl_id) as tot FROM audit_adset WHERE geo_locations!='' group by geo_locations");
		while($row5 = mysqli_fetch_array($sql_5)) { $geo_locations[]  = $row5['geo_locations'].' : '.$row5['tot']; }
		
		$sql_6 = mysqli_query($conn, "SELECT count(tbl_id) as tot FROM audit_adset WHERE daily_budget!=0 AND daily_budget!=''");
		while($row6 = mysqli_fetch_array($sql_6)) { $daily_budget  = 'DAILY : '.$row6['tot']; }
		
		$sql_7 = mysqli_query($conn, "SELECT count(tbl_id) as tot FROM audit_adset WHERE pacing_type=0 AND lifetime_budget!=''");
		while($row7 = mysqli_fetch_array($sql_7)) { $lifetime_budget  = 'LIFETIME : '.$row7['tot']; }
		
		$sql_8 = mysqli_query($conn, "SELECT interests FROM `audit_adset` WHERE interests!=''");
		...
		$sql_9 = mysqli_query($conn, "SELECT work_employers FROM `audit_adset` WHERE work_employers!=''");
		...
		$sql_10 = mysqli_query($conn, "SELECT work_positions FROM `audit_adset` WHERE interests!=''");
...
function LeadQus($url, $conn, $accId, $pgId) { 
				...
				if(isset($res['questions'])) {
					foreach ($res['questions'] as $k => $v) {
						if(isset($v['type']) && $v['type']=='CUSTOM'){
							...
							$cirRes = mysqli_query($conn, "select * from audit_lead_qus WHERE pg_id='".$pgId."' AND l_id='".$v['id']."'");						
	...
	if(isset($fb_response->paging->next)) {
		LeadQus($fb_response->paging->next, $accId, $pgId);
	} else {
		$leadQus = array();
		...
		$cirSql = "UPDATE audit_data SET lead_qus='".mysqli_real_escape_string($conn, $leadQus)."', updated=now() WHERE acc_id='".$accId."'";
		mysqli_query($conn, $cirSql) or die(mysqli_error());
	}
}

$lp_urls = array();
function Lp_Url($url, $conn, $accId, $pgId) { 
	...
	if(isset($fb_response->paging->next)) {
		Lp_Url($fb_response->paging->next, $accId, $pgId);
	} else {
		$lp_urls = array_unique($lp_urls);
		$lp_urls = array_filter($lp_urls);  
		$lp_urls = array_values($lp_urls); 
		$cirSql = "UPDATE audit_data SET lp_url='".mysqli_real_escape_string($conn, implode('<>',$lp_urls))."', updated=now() WHERE acc_id='".$accId."'";
		mysqli_query($conn, $cirSql) or die(mysqli_error());
		// d($lp_urls);
	}
}
...
function AdSetInsights($url, $conn, $accId, $obj_arr) { 
	...
	foreach ($fb_response['data'] as $key => $res) {
			...
			//if(mysqli_num_rows($cirRes)==0) {
					$cirSql = "INSERT INTO audit_ad (ad_id, adset_id, campaign_id, ad_type, ad_qty, obj_type, spend, lead, cpl, status, updated) VALUES ('".mysqli_real_escape_string($conn, $res['id'])."', '".mysqli_real_escape_string($conn, $res['adset_id'])."', '".mysqli_real_escape_string($conn, $res['campaign_id'])."', '".mysqli_real_escape_string($conn, $adType)."', '".mysqli_real_escape_string($conn, $adQty)."', '".mysqli_real_escape_string($conn, $objType)."', '".mysqli_real_escape_string($conn, $adSpend)."', '".mysqli_real_escape_string($conn, $lead)."', '".mysqli_real_escape_string($conn, $cpr)."', '".mysqli_real_escape_string($conn, $res['status'])."', now());"; 
					mysqli_query($conn, $cirSql) or die(mysqli_error()); //exit;
			//	} 
	}  
	if(isset($fb_response['paging']['next'])) {
		AdSetInsights($fb_response['paging']['next'], $conn, $accId, $obj_arr);
	} else {
		
		$ad_type_res = $ad_type_best = $ad_qty = array();
		$sql_1 = mysqli_query($conn, "SELECT ad_type, SUM(spend) AS spend, SUM(lead) as lead, (spend/lead) as per FROM `audit_ad` GROUP BY ad_type");
		while($row1 = mysqli_fetch_array($sql_1)) { $cpl=@($row1['spend']/$row1['lead']); if($cpl>0) { $ad_type_res[]  = $row1['ad_type'].' : '.round($cpl,2); } }
		...
		$sql_2 = mysqli_query($conn, "SELECT ad_type, min(cpl) AS cpl FROM `audit_ad` WHERE cpl!='0' GROUP BY ad_type");
		...
		$sql_3 = mysqli_query($conn, "SELECT ad_qty, count(ad_qty) AS tot FROM `audit_ad` WHERE ad_qty!='' GROUP BY ad_qty");
		...
		$cirSql = "UPDATE audit_data SET ad_type_res='".mysqli_real_escape_string($conn, implode('<>', $ad_type_res))."', ad_type_best='".mysqli_real_escape_string($conn, implode('<>', $ad_type_best))."', ad_qty='".mysqli_real_escape_string($conn, implode('<>', $ad_qty))."', updated=now() WHERE acc_id='".$accId."'";
		mysqli_query($conn, $cirSql) or die(mysqli_error());
	}
}

		
function Campaign($url, $conn, $accId, $obj_arr) { 
	...
	foreach ($fb_response['data'] as $key => $res) {
			...
			//if(mysqli_num_rows($cirRes)==0) {
					$cirSql = "INSERT INTO audit_campaign (id, name, objective, budget_type, spend, leads, cpl, adset_tot, status, updated) VALUES ('".mysqli_real_escape_string($conn, $res['id'])."', '".mysqli_real_escape_string($conn, $res['name'])."', '".mysqli_real_escape_string($conn, $objType)."', '".mysqli_real_escape_string($conn, $budget)."', '".mysqli_real_escape_string($conn, $cSpend)."', '".mysqli_real_escape_string($conn, $lead)."', '".mysqli_real_escape_string($conn, $cpr)."', '".mysqli_real_escape_string($conn, $adset_tot)."', '".mysqli_real_escape_string($conn, $res['status'])."', now());"; 
					mysqli_query($conn, $cirSql) or die(mysqli_error()); 
			//	} 
	}  //exit;
	if(isset($fb_response['paging']['next'])) {
		Campaign($fb_response['paging']['next'], $conn, $accId, $obj_arr);
	} else {
		
		
		$ad_type_res = $ad_type_best = $cmp_high = array();
		$objQ ='';
		$sql_1 = mysqli_query($conn, "SELECT tbl_id FROM `audit_campaign` WHERE adset_tot>1 AND budget_type='Yes'");
		if(mysqli_num_rows($sql_1)>0) { $cbo_camp ='Yes'; } else { $cbo_camp ='No'; }
		//SELECT id,name,objective, SUM(spend) as a, SUM(leads) as b, (SUM(spend)/SUM(leads)) as c FROM `audit_campaign` GROUP by objective
		
		$sql_2 = mysqli_query($conn, "SELECT id, name, objective, SUM(spend) as a, SUM(leads) as b, (SUM(spend)/SUM(leads)) as c FROM `audit_campaign` GROUP by objective");
		
		while($row2 = mysqli_fetch_array($sql_2)) {  if($row2['c']>0) { $objQ .= " (objective ='".$row2['objective']."' AND cpl>".round($row2['c'],2).") OR "; } }
		if($objQ!='') {
			$objQ = ' WHERE '.substr($objQ, 0, -4) ;
		...
		$cirSql = "UPDATE audit_data SET cbo_camp='".mysqli_real_escape_string($conn, $cbo_camp)."', high_cpl='".mysqli_real_escape_string($conn, implode('<>', $cmp_high))."', updated=now() WHERE acc_id='".$accId."'";
		mysqli_query($conn, $cirSql) or die(mysqli_error());
	...
	if($sqlROW['fb_id']!='') 
	{
		$fbIds = explode(',',$sqlROW['fb_id']);	$fb_stDt = explode(',',$sqlROW['fb_stDt']);	
		foreach($fbIds as $key => $fbId) {						
			...
			ObjResults($request_url, $conn, $sqlROW['fb_id'], $page_id, $obj_arr);
			...
			mysqli_query($conn, "UPDATE audit_data SET age='".$created_time."', active='".$fb_response['account_status']."' WHERE acc_id=".$fbId."") or die(mysqli_error());  
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			mysqli_query($conn, "UPDATE audit_data SET camp_tot='".$fb_response['summary']['total_count']."' WHERE acc_id=".$fbId."") or die(mysqli_error());  
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if($fb_response['data'][0]['is_unavailable']=='' || $fb_response['data'][0]['is_unavailable']==false) { $pix_act = 'Yes'; } else { $pix_act = 'No'; }
			mysqli_query($conn, "UPDATE audit_data SET pix_act='".$pix_act."' WHERE acc_id=".$fbId."") or die(mysqli_error());  
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(count($fb_response['data'])>0) { $cust_conv='Yes'; } else { $cust_conv='No'; }
			mysqli_query($conn, "UPDATE audit_data SET cust_conv='".$cust_conv."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			mysqli_query($conn, "UPDATE audit_data SET rules='".$rules."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(count($fb_response['data'])>0) { $cust_aud='Yes'; } else { $cust_aud='No'; }
			mysqli_query($conn, "UPDATE audit_data SET cust_aud='".$cust_aud."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(count($fb_response['data'])>0) { $la_aud='Yes'; } else { $la_aud='No'; }
			mysqli_query($conn, "UPDATE audit_data SET la_aud='".$la_aud."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(count($fb_response['data'])>0) { $remarket='Yes'; } else { $remarket='No'; }
			mysqli_query($conn, "UPDATE audit_data SET remarket='".$remarket."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(count($fb_response['data'])>0) { $top_web_25='Yes'; } else { $top_web_25='No'; }
			mysqli_query($conn, "UPDATE audit_data SET top_web_25='".$top_web_25."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(count($fb_response['data'])>0) { $top_la_25='Yes'; } else { $top_la_25='No'; }
			mysqli_query($conn, "UPDATE audit_data SET top_la_25='".$top_la_25."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			$requests = curl_get_file_contents($request_url);
			$fb_response = json_decode($requests,true);
			if(isset($fb_response['instagram_accounts']['data']) && count($fb_response['instagram_accounts']['data'])>0) { $insta_acc='Yes'; $ig_count=$fb_response['instagram_accounts']['data'][0]['followed_by_count']; } else { $insta_acc='No'; $ig_count=0; }
			mysqli_query($conn, "UPDATE audit_data SET insta_acc='".$insta_acc."',ig_followers='".$ig_count."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			exit;
			...
			mysqli_query($conn, "UPDATE audit_data SET img_updated='".$img_updated."' WHERE acc_id=".$fbId."") or die(mysqli_error());
			...
			AdSet($request_url, $conn, $sqlROW['fb_id']); //exit;
			...
			//LeadQus($request_url, $conn, $sqlROW['fb_id'], $page_id);
			...
			//Get LP URLs 
			$request_url = "https://graph.facebook.com/".$api_ver."/act_".$sqlROW['fb_id']."/adsets?fields=adcreatives{object_story_spec{link_data{link}}}&filtering=[{'field':'campaign.objective','operator':'IN','value':['CONVERSIONS']}]&access_token=".$pg_access_token."&limit=100";
			Lp_Url($request_url, $conn, $sqlROW['fb_id'], $page_id);
			...
			AdSetInsights($request_url, $conn, $fbId, $obj_arr);
			*/
			...
			Campaign($request_url, $conn, $fbId, $obj_arr);
