...
class ParallelReportDownload
{
    ...
    public static function runExample(
        ...
        $selector->setFields(
            ['CampaignId', 'CampaignName', 'AverageCpc', 'Impressions', 'Clicks', 'Cost', 'AverageCost', 'AverageCpm', 'CostPerConversion', 'Ctr', 'Conversions']
        );
...
foreach ($weeks as $key => $value)  {
	...
	$fromDt = date('Ymd', $weeks[$key][0]);
	$enDt = date('Ymd', $weeks[$key][1]);
	
	$gIds = array();
	//d($accIds); exit;	
	$sqlD = mysqli_query($conn, "select acc_id, report_1 from ads_weekly WHERE uid='2' AND acc_type='G' AND acc_id IN (".implode(',',$accIds).") AND st_dt='".$weeks[$key][0]."' AND en_dt='".$weeks[$key][1]."'");
