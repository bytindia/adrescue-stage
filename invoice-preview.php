...
function cleanText($text) {
    $replacements = [
        ...
        '…' => '...',
        '•' => '-',   // bullet
        '₹' => 'Rs.', // fallback for rupee symbol
    ];
    $text = strtr($text, $replacements);

    // Then, convert to ISO-8859-1, replacing unknowns with a blank
...
$inv_dt = date('d-m-Y'); 
if($data['inv_dt']!='') { $inv_dt = $data['inv_dt']; }
...
if(isset($_POST['submit']))
{
    ...
    if(isset($inv_nos[3]) && $inv_nos[1]!='') { $invN = $inv_nos[3]; }
    if(isset($inv_nos[4]) && $inv_nos[2]!='') { $invN = $inv_nos[4]; }

    $inv_pi = isset($_GET['pi']) ? 'pi' : '';
    mysqli_query($conn, "UPDATE invoice2 SET approved='yes', inv_ty='".$inv_pi."' where tbl_id=".$data['tbl_id']."");
    ...
    foreach($line_items as $key => $lineItems){
        ...
        $leadV = [[$invNo, date('d-m-Y'), mysqli_real_escape_string($conn, $data['gst_no']), mysqli_real_escape_string($conn, cleanText($data['client_name'])), mysqli_real_escape_string($conn, cleanText($lineItems['line_items'][0]['label'])), $lineItems['totals']['subtotal'], $lineItems['totals']['cgst'], $lineItems['totals']['sgst'], $lineItems['totals']['igst'], $lineItems['totals']['grand_total'], $tds, $tds_amt, $receivable,'', '', '']];

        $leadV_soa = [[
          $invNo,
          date('d-m-Y'),
          
          mysqli_real_escape_string($conn, cleanText($data['client_name'])),
          mysqli_real_escape_string($conn, cleanText($lineItems['line_items'][0]['label'])),
        ...
        foreach($lineItems['line_items'] as $k => $v){
            ...
            $leadV1[] = [$invNo, date('d-m-Y'), mysqli_real_escape_string($conn, $data['gst_no']), mysqli_real_escape_string($conn, cleanText($data['client_name'])), mysqli_real_escape_string($conn, cleanText($v['label'])),$sub_tot, $cgst_amt, $sgst_amt, $igst_amt, $total_with_gst, $tds, $tds_amt, $receivable,'', '', ''];
            $leadV2[] = [$invNo, date('d-m-Y'), mysqli_real_escape_string($conn, cleanText($data['client_name'])), mysqli_real_escape_string($conn, cleanText($v['label'])),$sub_tot, $cgst_amt, $sgst_amt, $igst_amt, $total_with_gst, $tds, $tds_amt, $receivable,'', '', ''];
        ...
        if(count($leadV2)>0) {
          ...
          if(!in_array($invTy, ['pi', 'pi-2', 'pi-3'])) { 
            append_to_sheet($uId, $tbl_id2, $leadV_soa, $sheetId, $data['gsheet']); 
          } else {
            append_to_sheet($uId, $tbl_id2, $leadV_soa, $sheetId, $data['gsheet'].' - PI'); //PI sheet
          } //SOA sheet
        }
   ...
   // $leadV = [[mysqli_real_escape_string($conn, $lead['name']),$lead['email'],$ph, $src, $med, $camp, date('d-m-Y, h:i a')]];
    //exit;
    if((!empty($_POST['soa_file']) || !empty($_POST['soa_pi_file']))) { include 'invoice-soa-inc.php'; } // show soa
    include 'email/invoice2025.php';
    $_SESSION['suc'] = "Invoice has been sent!";
    echo "<script>window.location = 'wa-preview.php?id=$tbl_id3';</script>"; exit;
  ...
  if(isset($_GET['pi']))  { $pi_soa = 'yes'; } else { $pi_soa = 'no'; }
  include 'invoice-soa-inc.php';  // show soa
}
...
#previewModal .modal-body {
  ...
  overflow: hidden;
    ...
    /* Ensure that buttons inside btn-group align properly */
    .btn-group {
      display: inline-flex;  /* Align buttons side by side */
      align-items: center;   /* Vertically align items */
      /* Add margin between button groups */
    }

    .btn-group .btn {
      margin-right: 0;  /* Remove any margin between buttons */
    }

    /* Optional: Adjust the padding to make buttons more compact */
    .btn-group .btn-sm {
      padding: 5px 10px;  /* Reduce padding if needed */
    }

    .fas.fa-trash {
      font-size: 14px;  /* Adjust the size of the trash icon */
      margin-left: 5px;  /* Space between the button and the trash icon */
    }
.mar-left {
    margin-left: 10px;
}
...
// Prepare details block for insertion into textarea
 $inv_type = 'BYT/SW/25/'; $inv_ty_msg = ''; $inv_ty_soa = 'and SOA ';
...
foreach ($inv_nos as $i => $inv_no) {
    ...
    $detailsBlock .= "{$z}. {$inv_type}{$inv_no} | {$inv_dt} | Rs. " . moneyFormatIndia($amount) . " (incl. taxes)";
...
​accounts@bytindia.com
      ...
      <div class="col-md-5">
        ...
        <div class="attachment-buttons">
          <?php $in = 1; foreach ($inv_files as $in_k => $file): ?>
            <?php if (trim($file)): ?>
              <button type="button" class="btn btn-outline-primary btn-sm"
                onclick="previewAttachment('https://stage.adrescue.in/fb-ads/demo/invoices/<?php echo trim($file); ?>.pdf')"
                data-bs-toggle="modal" data-bs-target="#previewModal">Invoice # <?php echo $inv_nos[$in_k]; //$in++; ?></button>
            <?php endif; ?>
          <?php endforeach; ?>

          <?php if (!empty($data['report_files'])): ?>
            <div id="report_file" class="btn-group">
              <button type="button" class="btn btn-outline-primary btn-sm"
              ...
              <button type="button" class="btn btn-outline-primary btn-sm" onclick="removeFile('report_file', 'report_files')"  data-bs-toggle="tooltip" title="Remove AdReport?">
                <i class="fa-solid fa-close">‌</i>
              </button>
            </div>
          <?php endif; ?>
          <input type="hidden" name="report_files" value="<?php if (!empty($data['report_files'])): echo $data['report_files']; endif; ?>">
          ...
          <div class="mb-3 d-flex " style="margin-top: 220px; float: right;">
            <?php if($inv_check=='yes') { ?>
                ...
                <button type="submit" name="submit" id="submitBtn" value="Submit" class="btn btn-primary btn-md" onclick="return confirmAndSubmit();" >
                  Approve & Send Email
                </button>
            <?php } ?>
          </div>
...
function confirmAndSubmit() {
 ...
 function removeFile(fileId, fileInputName) {
  ...
  if (['docx', 'pptx'].includes(ext)) {
    ...
    finalUrl = url + '#navpanes=0&scrollbar=0'; //finalUrl = url + '#toolbar=0&navpanes=0&scrollbar=0';
  } else {
