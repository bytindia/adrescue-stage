         ...
         <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <div class="x_title">
                    <h2><?php echo $cName; ?> - Campaign wise Reports</h2>
                  		...
                  		<?php 
                                ...
                                <table id="datatable" class="table table-hover table-striped table-bordered">
                                    ...
                                    <tbody>
										...
										//echo 'https://graph.facebook.com/'.$api_ver.'/act.$_GET['id'].'/insights?level=campaign&fields=campaign_name,campaign_id,spend,reach,impressions,account_currency&access_token='.$access_token.'&time_range[since]='.date("Y-m-d", strtotime($_SESSION['stDt'])).'&time_range[until]='.date("Y-m-d", strtotime($_SESSION['enDt'])).''.$after;
										$val = get_data('https://graph.facebook.com/'.$api_ver.'/act'.$_GET['id'].'/insights?level=campaign&fields=account_name,campaign_name,campaign_id,spend,reach,impressions,account_currency&&filtering=[{"field":"campaign.impressions","operator":"GREATER_THAN","value":0}]&access_token='.$access_token.'&time_range[since]='.date("Y-m-d", strtotime($_SESSION['stDt'])).'&time_range[until]='.date("Y-m-d", strtotime($_SESSION['enDt'])).''.$after);
										//d($val);	
										//echo count($val['data']); 
										if(count($val['data'])==0) 
										{ 
											...
											<tr>
												<td><?php echo $i; ?></td>
												<td><a href="loading.php?pg=adset.php?id=<?php echo $cS['campaign_id']; ?>&camp=<?php echo $cS['campaign_name']; ?>" target="_blank" class="blue"><?php echo $cS['campaign_name']; ?></a></td>
												<td><?php echo $cS['campaign_id']; ?></td>
