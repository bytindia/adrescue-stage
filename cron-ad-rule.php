...
$sqlRev=mysqli_query($conn, "SELECT tbl_id,client_name,fb_id,fb_stDt,fb_enDt,g_id,g_stDt,in_id,in_stDt,ta_id,ta_stDt,fb_received FROM cashflow2 WHERE $extQ uid='".$uId."' AND delete_status=0");
...
while($sqlROW=mysqli_fetch_array($sqlRev))
{
	...
	if($sqlROW['fb_id']!='') 
	{
		...
		//echo "UPDATE cashflow2 SET fb_spent='".implode(',',$fbSpent)."' WHERE tbl_id=".$sqlROW['tbl_id'].""; 
		if(!isset($_GET['ajax'])) { 
			mysqli_query($conn, "UPDATE cashflow2 SET fb_spent='".implode(',',$fbSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); //exit;
		}
	}
	//exit;
	//Google
	if($sqlROW['g_id']!='') {
		...
		if(!isset($_GET['ajax'])) {
			mysqli_query($conn, "UPDATE cashflow2 SET g_spent='".implode(',',$gSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());	
		}
	}
	...
	if($sqlROW['in_id']!='') { 
		...
		if(!isset($_GET['ajax'])) {
			mysqli_query($conn, "UPDATE cashflow2 SET in_spent='".implode(',',$inSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());		
		}
	}
	//exit;
	//Taboola
	if($sqlROW['ta_id']!='' && $taboola_tok!='') {
		...
		if(!isset($_GET['ajax'])) {
			mysqli_query($conn, "UPDATE cashflow2 SET ta_spent='".implode(',',$taSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());	
		}
...
$sqlRev=mysqli_query($conn, "SELECT c.*, cp.amount, cp.date FROM cashflow2 as c, cashflow_payments2 as cp WHERE c.tbl_id=cp.cashflow_id AND c.uid='2' AND c.delete_status=0 order by c.tbl_id asc");
