...
if($client_id == 0) {
    // Show client selection page instead of error
    ?>
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Select Client - Keyword Dashboard</title>
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="mb-0">Select Client for Keyword Dashboard</h4>
                        </div>
                        <div class="card-body">
                            <div class="form-group">
                                <label for="clientDropdown">Select Client:</label>
                                <select id="clientDropdown" class="form-control" onchange="if(this.value) window.location.href=this.value;">
                                    <option value="">Choose a client...</option>
                                    <?php
                                    // Query to get all clients for current user
                                    ...
                                    while($clientRow = mysqli_fetch_array($clientQuery)) {
                                        echo '<option value="loading.php?pg=keywords.php&tbl_id='.$clientRow['tbl_id'].'&date='.urlencode(date('d/m/Y')).'">'.htmlspecialchars($clientRow['client_name']).'</option>';
                                    }
                                    ?>
                                </select>
                            </div>
...
<div class="container mt-4">
    <div class="date-filter-row pull-right">
        <form method="get" class="form-inline" style="display: flex; align-items: center;">
            <!-- Client Dropdown -->
            <label for="clientDropdown" class="form-label mb-0" style="margin-right: 8px;">Select Client:</label>
            <select id="clientDropdown" class="form-control" style="display: inline-block; width: auto; min-width: 150px; margin-right: 8px;" onchange="if(this.value) window.location.href=this.value;">
                <option value="">Select Client</option>
                <?php
                // Query to get all clients for current user
                $clientQuery = mysqli_query($conn, "SELECT tbl_id, client_name FROM dashboard_accounts WHERE uid='".$_SESSION['uid']."' AND delete_status=0 ORDER BY client_name ASC");
                while($clientRow = mysqli_fetch_array($clientQuery)) {
                    $selected = ($client_id == $clientRow['tbl_id']) ? 'selected' : '';
                    echo '<option value="loading.php?pg=keywords.php&tbl_id='.$clientRow['tbl_id'].'&date='.urlencode($selected_date).'" '.$selected.'>'.htmlspecialchars($clientRow['client_name']).'</option>';
                }
                ?>
            </select>
            
            <label for="date" class="form-label mb-0" style="margin-right: 8px;">Select Date:</label>
...
$(function() {
    ...
    $('#date').on('apply.daterangepicker', function(ev, picker) {
        ...
        if (clientId) {
            var url = 'loading.php?pg=keywords.php&tbl_id=' + clientId + '&date=' + encodeURIComponent(date);
    ...
    changeModalTitle("Keywords Dashboard: <?php echo htmlspecialchars($client_data['client_name']); ?>");
