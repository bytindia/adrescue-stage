...
$d2 = new DateTime('last day of last month');
...
mysqli_query($conn, 'TRUNCATE TABLE budget_reminder_last_mon;') or die(mysqli_error()); 
mysqli_query($conn, "INSERT INTO budget_reminder_last_mon SELECT * FROM budget_reminder") or die(mysqli_error()); 

$sqlRev=mysqli_query($conn, "SELECT tbl_id,fb_id,fb_stDt,g_id,g_stDt,in_id,in_stDt,ta_id,ta_stDt,fb_received FROM budget_reminder_last_mon  WHERE $extQ uid='".$uId."' AND delete_status=0 AND hide_temp='0'");
...
while($sqlROW=mysqli_fetch_array($sqlRev))
{
	...
	mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET fb_spent='0', fb_balance='', g_spent='0', in_spent='0', ta_spent='0' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); 
	...
	if($sqlROW['fb_id']!='') 
	{
		...
		foreach($fbIds as $key => $fbId) {						
		    ...
		    $dtRange = 'time_range[since]='.date("Y-m-d",strtotime($SatrtDate)).'&time_range[until]='.date("Y-m-d", strtotime($EndDate)).''; //exit;
			 ...
			 $request_url = 'https://graph.facebook.com/'.$api_ver.'/act_'.$fbId.'/insights?level=account&fields=spend,actions,cost_per_action_type&access_token='.$access_token.'&'.$dtRange.''; //exit;
            ...
            $request_url2 = 'https://graph.facebook.com/'.$api_ver.'/act_'.$fbId.'/insights?level=account&fields=spend,actions,cost_per_action_type&access_token='.$access_token.'&date_preset=last_month'; //exit;
...
/*
		...
		//echo "UPDATE budget_reminder_last_mon  SET fb_spent='".implode(',',$fbSpent)."', fb_balance='".implode(',',$acc_bal)."' WHERE tbl_id=".$sqlROW['tbl_id'].""; exit;
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET fb_spent='".implode(',',$fbSpent)."', fb_leads='".implode(',',$fblead)."', fb_cpl='".implode(',',$fbcpl)."', fb_leads2='".implode(',',$fblead2)."', fb_cpl2='".implode(',',$fbcpl2)."', fb_balance='".implode(',',$acc_bal)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); 
	}
	//exit;
	//Google
	...
	if($sqlROW['g_id']!='') {
		...
		//echo "UPDATE budget_reminder_last_mon  SET g_spent='".implode(',',$gSpent)."' WHERE tbl_id=".$sqlROW['tbl_id'].""; exit;
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET g_spent='".implode(',',$gSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); 
	}
	//exit;
	//LinkedIn
	if($sqlROW['in_id']!='') { 
		...
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET in_spent='".implode(',',$inSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());		
	}
	//exit;
	//Taboola
	if($sqlROW['ta_id']!='' && $taboola_tok!='') {
		...
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET ta_spent='".implode(',',$taSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());	
		//d($ta_res); 
...
$sqlRev=mysqli_query($conn, "SELECT tbl_id,fb_id,fb_stDt,g_id,g_stDt,in_id,in_stDt,ta_id,ta_stDt,fb_received FROM budget_reminder_last_mon  WHERE $extQ uid='".$uId."' AND delete_status=0");
//$sqlROW=mysqli_fetch_assoc($sqlRev);
//d($sqlROW); exit;			
$total_spent = 0; 					
while($sqlROW=mysqli_fetch_array($sqlRev))
{
	...
	if($sqlROW['fb_id']!='') 
	{
		...
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET fb_spent_y='".implode(',',$fbSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); 
	}
	...
	if($sqlROW['g_id']!='') {
		...
		//echo "UPDATE budget_reminder_last_mon  SET g_spent='".implode(',',$gSpent)."' WHERE tbl_id=".$sqlROW['tbl_id'].""; exit;
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET g_spent_y='".implode(',',$gSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); 
	}
	...
	if($sqlROW['in_id']!='') { 
		...
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET in_spent_y='".implode(',',$inSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());		
	}
	//exit;
	//Taboola
	if($sqlROW['ta_id']!='' && $taboola_tok!='') {
		...
		mysqli_query($conn, "UPDATE budget_reminder_last_mon  SET ta_spent_y='".implode(',',$taSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());	
...
//exit;
//EMAIL REPORT
//$sqlRev=mysqli_query($conn, "SELECT * FROM budget_reminder_last_mon  WHERE uid='2' AND delete_status=0 order by tbl_id asc");
//echo "SELECT c.*, cp.amount, cp.date FROM budget_reminder_last_mon  as c, cashflow_payments as cp WHERE c.tbl_id=cp.cashflow_id AND c.uid='2' AND c.delete_status=0 order by c.tbl_id asc"; 
$sqlRev=mysqli_query($conn, "SELECT * FROM budget_reminder_last_mon  WHERE uid='2' AND delete_status=0 order by tbl_id asc");
...
if(isset($_GET['refresh'])) {
	echo "<script>window.location = 'budget4-2.php';</script>"; 
	exit();
} else {
