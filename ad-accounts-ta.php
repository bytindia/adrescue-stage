...
Auth();
$pgHeadline = 'Taboola - Ad Account List';
$pgID = 3;
$err =''; 

if(isset($_POST['submit'])){
	
	$cirSql = "UPDATE adAccounts_ta SET status='0', updated=now() where uid='".$_SESSION['uid']."'";
	mysqli_query($conn, $cirSql) or die(mysqli_error()); 
	
	if(isset($_POST['ads_id']) && count($_POST['ads_id'])>0) {
		//echo (count($_POST['ads_id'])); 
		$ids = implode(",", $_POST['ads_id']);
		$cirSql2 = "UPDATE adAccounts_ta SET status='1', updated=now() where tbl_id in (".$ids.") && uid='".$_SESSION['uid']."'";
...
$statement = " adAccounts_ta WHERE uid='".$_SESSION['uid']."'";
                    ...
                    <ul class="nav navbar-right panel_toolbox">
                      ...
                      <li> &nbsp;
                       <div class="btn-group  btn-group-sm">
                        	<a href="loading.php?pg=adAccounts-ta.php"  class="btn btn-primary btn-sm">Sync New Accounts</a>                
                      	</div>  
                      </li>
