...
class GetCampaigns
{
    ...
    public static function main($conn,$g_ref_tok,$g_mcc,$adAccId, $st, $en)
    {
		...
		public static function runExample(GoogleAdsClient $googleAdsClient, int $customerId, $st, $en)
		{
			...
			metrics.conversions,
			segments.date
			FROM campaign WHERE segments.date >= '".$st."' AND segments.date <= '".$en."'  $filtering_param_g ORDER BY campaign.id";  
			...
			echo array_sum($conv);*/
			return array(
				'cost'=>array_sum($cost), 'conversions'=>array_sum($conv), 'clicks'=>array_sum($clicks), 'ctr'=>array_sum($ctr), 'impressions'=>array_sum($impressions)
			);
		}
}
