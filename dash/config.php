...
$cirRes = mysqli_query($conn, "SELECT da.tbl_id, da.client_name, da.fb_id, da.g_id, da.in_id, da.ta_id, da.proj_name, da.name_contain, br.total_budget FROM dashboard_accounts da LEFT JOIN budget_reminder br ON da.bud_id = br.tbl_id WHERE da.tbl_id = '".mysqli_real_escape_string($conn, $_GET['tbl_id'])."'");
$row = mysqli_fetch_assoc($cirRes);
$_GET['tbl_id'] = $row['tbl_id'];
...
if($row['proj_name']!='') { $proj_names = array_filter(unserialize($row['proj_name'])); }
if($row['name_contain']!='') { $name_contain = $name_contain2 = array_filter(unserialize($row['name_contain'])); }
$total_budget =0;
if($row['total_budget']!='') { $total_budget = $row['total_budget']; }
...
if(is_array( $g_acc_ids ) && count($g_acc_ids)>0) {
    ...
    /*function contains_proj($str)
    {
        ...
        global $name_contain;
        //if(count($name_contain)>0) { return 'no'; }
        foreach($name_contain as $k => $a) { 
            $a = strtolower(trim($a));
            if (stripos($str,$a) !== false) return $k;
        }
        return '';
    }*/
    function contains_proj($str) {
    ...
    function contains_proj2($str)
    {
        return 'no';
    }
    
    
$ad_acc_all = array(732236261026706,665661671485180);
    
$acTypes = array('RWD');
$adAccIds = array(
    'RWD'=> array(732236261026706,665661671485180)
);
$budTy = array('RWD'=>1,'NRI'=>2);
$budTy_G = array('RWD'=>3,'NRI'=>4);

$ad_acc_all_g = array('9516894033');
$adAccIds_g = array(
    'RWD'=> array('9516894033')
);
