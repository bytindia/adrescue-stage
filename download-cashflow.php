...
class ParallelReportDownload
{
    ...
    public static function main($ClientIds, $fromDt, $enDt, $g_mcc, $g_refresh_token, $server_path)
    {
...
{
	if($gDates[$gR]!='') {
		$fromDt = date('Ymd', strtotime($gDates[$gR]));
	} else {
		$fromDt = 1;
	}	
	$enDt = date('Ymd', strtotime($today)); 
	ParallelReportDownload::main(array($gR), $fromDt, $enDt, $g_mcc, $g_refresh_token, $server_path);
}
