...
if(isset($_POST['submit'])){
    //d($_POST); exit;
    $adsets_ids = serialize($_POST['adsets']);
    $cust_conv_id ='';
    if(isset($_POST['cust_con_check']) && isset($_POST['cust_con']) && $_POST['cust_con']!='') {
        $cust_conv_id = $_POST['cust_con'];
    }
    $cirSql = "INSERT INTO adset_report (acc_id, user_id, acc_name, obj, adset_ids, cust_conv, created) VALUES ('".$_POST['category']."', '".$user_id."', '".$_POST['client']."', '".$_POST['obj']."', '".mysqli_real_escape_string($conn, $adsets_ids)."', '".mysqli_real_escape_string($conn, $cust_conv_id)."', now());"; 
...
<a href="loading.php?pg=accounts.php" class="btn btn-sm btn-warning float-right">View Your Accounts</a>
    ...
    <label for="example" id="multiselectLabel" class="mt-3" style="display:none;">Select AdSets:</label>
    <select name="adsets[]" id="example" multiple="multiple" class="form-control" style="display:none;" required>
...
$(document).ready(function() {
    ...
    $('#category').change(function() {
        ...
        if (selectedCategory) {
            ...
            $('#example').hide();
            $('#example').empty();
            $('#multiselectLabel').html('Fetching Adsets...');

            // Make an Ajax request to fetch additional options based on category
            $.ajax({
                url: 'get_adsets_data.php', // PHP endpoint to fetch options
                type: 'GET',
                dataType: 'json',
                data: { category: selectedCategory },
                success: function(data) {
                    // Clear any existing options
                    $('#multiselectLabel').html('Select AdSets:');
                    $('#example').show();
