...
$spreadsheetId = '1go3zfOeKebTl2HDrakdf72QRw7WRyoMHdCO70RtjP3c';
$sheetTab = 'Sheet1';
$uid_sheet = 2;
...
unset($fetch_sale['data'][0]);
//echo '<pre>';   print_r($fetch_sale); echo '</pre>'; exit;
...
$filter_sale = array_filter($fetch_sale['data'], function($var) use ($rangeStart, $rangeEnd) {  
    //$dt = str_replace('/', '-', $var[1]); 
    $myDateTime = DateTime::createFromFormat('d/m/y', $var[1]);
    $dt = $myDateTime->format('Y-m-d');
    $evtime = strtotime($dt);  
...
$form_label = array('First Name','Last Name','Email','Phone','UTM Source','UTM Medium','UTM Campaign','Created');



$_POST['daterange'] = date("m/d/Y", strtotime($dtRange1)).' - '.date("m/d/Y", strtotime($dtRange2));

$post = [
    "formKey" => "bytAzadi2023", 
    "daterange" => $_POST['daterange']
];

$url = 'https://digitalazadi.com/da-vlookup/export.php';
$ch = curl_init($url); 
...
$saleData = array();
if(count($result['data1'])>0 && count($result['data2'])){
    $saleData = array_merge($result['data1'], $result['data2']);
} else if(count($result['data1'])>0) {
    $saleData = $result['data1'];
} else if(count($result['data2'])>0) {
    $saleData = $result['data2'];
} 
...
foreach($saleData as $k => $v){
    ...
    $src_val = trim($v[4]);
    $cmp_val = trim($v[6]);
    $csvData[$src_val.'<>'.$cmp_val][] = trim($v[2]);
    $csvData3[$v[2]] = $src_val.'<>'.$cmp_val;
}
//d($csvData3); exit;
