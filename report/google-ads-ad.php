...
class GetAd
{
    ...
    public static function main($conn,$g_ref_tok,$g_mcc,$adAccId, $st, $en)
    {
		...
		public static function runExample(GoogleAdsClient $googleAdsClient, int $customerId, $st, $en)
		{
			...
			FROM ad_group_ad WHERE segments.date >= '".$st."' AND segments.date <= '".$en."' AND metrics.impressions>0 ORDER BY ad_group_ad.ad.id"; 
			...
			foreach ($stream->iterateAllElements() as $googleAdsRow) {
				...
				//$c = $ad->getAudienceSetting();
				//if(isset($c->getUseAudienceGrouped())) { $useAudienceGrouped = 1; }
				//print_r($c->getAudienceSetting());
				//var_dump($c->getUseAudienceGrouped());
				//echo $ad->getId().' --> '.$ad->getAudienceSetting().' <br>';
				//echo $metrics->getCostMicros().' --> '.$metrics->getConversions().'<br>';
				//$gSpent[] = round($cost);
				//echo '<br>';
				//print_r($adGroup);
                $adData[] = array($adId, $adName, $channelN[$adChannel], $statusN[$adStatus], $avgCpc, $impressions, $clicks, $cost, $avgCpc, $avgCpm, $CostPerConv, $ctr, $conv, $adGroup, $adGroupCriterion, $useAudienceGrouped);
