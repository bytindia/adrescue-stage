...
{       
        ...
        if(isset($fb_data[$k]) && count($fb_data[$k])>0) 
        {
            ...
            foreach($fb_data[$k] as $k_fb => $v_fb) {
                ...
                $camp_data_ri[$k][$v_fb['campaign_id']][] = array('spend'=> $v_fb['spend'], 'lead'=>$lead, 'camp_name'=>$v_fb['campaign_name']);
                $adset_data_ri[$k][$v_fb['adset_id']][] = array('spend'=>$v_fb['spend'], 'lead'=>$lead, 'camp_name'=>$v_fb['campaign_name']);
                $ad_data_ri[$k][$v_fb['ad_id']][] = array('spend'=>$v_fb['spend'], 'lead'=>$lead, 'camp_name'=>$v_fb['campaign_name']);
            }
