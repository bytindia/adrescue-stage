...
<?php if(!isset($header_no)) { ?>
  ...
  { 
    //$val = $acc_id;
     $url = "https://graph.facebook.com/".$api_ver."/act_".$k."/insights?level=account&breakdowns=publisher_platform,device_platform,platform_position&fields=adset_id,adset_name,reach,impressions,spend,objective,actions&filtering=[{'field':'campaign.objective','operator':'IN','value':['LEAD_GENERATION','OUTCOME_LEADS']}]&time_range[since]=".date("Y-m-d", strtotime($dtRange1))."&time_range[until]=".date("Y-m-d", strtotime($dtRange2))."&access_token=".$access_token."&limit=750";
...
<table id="datatable2" class="table table-hover table-striped table-bordered datatable" data-ordering="true">
    ...
    <tbody>
            ...
            <?php foreach($ad_acc as $k => $val)  { 
                ...
                <td><?php echo round(array_sum($spend_tot[$k])); ?></td>
                <td><?php echo round(array_sum($lead_tot[$k])); ?></td>
