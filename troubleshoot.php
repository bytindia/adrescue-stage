...
if(isset($_POST['dt_submit'])){
	...
	$_SESSION['stDt'] = $start ;
	$_SESSION['enDt'] = $end ;
	echo "<script>window.location =  'adset.php?id=".$_GET['id']."&camp=".$_GET['camp']."';</script>";
	exit();
}	
...
function get_data($url) {
	...
	$getID = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name FROM adAccounts WHERE id = '".$_GET['id']."'"));
	$cName = $getID['name'];
         ...
         <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <div class="x_title">
                    <h2>AdSets - <?php echo $cName; ?></h2>
                    <ul class="nav navbar-right panel_toolbox">
                      <li> &nbsp;
                      </li>
                      
                    </ul>
                    <form method="post" action="adset.php?id=<?php echo $_GET['id']; ?>&camp=<?php echo $_GET['camp']; ?>">
                    <ul class="nav navbar-right panel_toolbox">
                      <li>Filter : &nbsp;
                      </li>
                      <li>
                  		...
                  		<?php 
                                ...
                                <table id="datatable" class="table table-hover table-striped table-bordered">
                                    ...
                                    <tbody>
										...
										if(isset($_GET['pgId']) && isset($_GET['pgType'])) { $after='&'.$_GET['pgType'].'='.$_GET['pgId']; } else { $after=''; }
										//echo 'https://graph.facebook.com/'.$api_ver.'/act_'.$_GET['id'].'/insights?level=campaign&fields=campaign_name,campaign_id,spend,reach,impressions,account_currency&access_token='.$access_token.'&time_range[since]='.date("Y-m-d", strtotime($_SESSION['stDt'])).'&time_range[until]='.date("Y-m-d", strtotime($_SESSION['enDt'])).''.$after;
										$val = get_data('https://graph.facebook.com/'.$api_ver.'/'.$_GET['id'].'/insights?level=adset&fields=account_name,adset_name,adset_id,spend,reach,impressions,account_currency&&filtering=[{"field":"adset.impressions","operator":"GREATER_THAN","value":0}]&access_token='.$access_token.'&time_range[since]='.date("Y-m-d", strtotime($_SESSION['stDt'])).'&time_range[until]='.date("Y-m-d", strtotime($_SESSION['enDt'])).''.$after);
										//d($val);	
										//echo count($val['data']); 
										if(count($val['data'])==0) 
										{ 
											...
											<tr>
												<td><?php echo $i; ?></td>
												<td><a href="loading.php?pg=ad.php?id=<?php echo $cS['adset_id']; ?>&camp=<?php echo $cS['adset_name']; ?>" target="_blank" class="blue"><?php echo $cS['adset_name']; ?></a></td>
												<td><?php echo $cS['adset_id']; ?></td>
