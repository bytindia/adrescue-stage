...
if($_POST['ty']==1) {
			$tot_bal = $tot = 0;
			$sqlRev=mysqli_query($conn, "SELECT * FROM cashflow2 where tbl_id={$_POST['id']}");
			//$sqlRev=mysqli_query($conn, "SELECT * FROM cashflow as c, cashflow_payments2 as cp where c.tbl_id=9 AND c.tbl_id=cp.cashflow_id");
			...
			{ 
					...
					if($sqlROW["tot_paid"]!='') { echo '<tr><td>Payment Received (B)</td><td><i class="fa fa-inr"></i> '.money_format('%!i',$sqlROW["tot_paid"]).' </td></tr>'; }
...
if($_POST['ty']==2) {
			$sqlRev=mysqli_query($conn, "SELECT amount,date FROM cashflow_payments2 where cashflow_id={$_POST['id']} limit 0,1");
