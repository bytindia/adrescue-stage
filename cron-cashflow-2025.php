...
$sqlRev=mysqli_query($conn, "SELECT tbl_id,client_name,fb_id,fb_stDt,fb_enDt,g_id,g_stDt,in_id,in_stDt,ta_id,ta_stDt,fb_received FROM cashflow2024 WHERE $extQ uid='".$uId."' AND delete_status=0");
//$sqlROW=mysqli_fetch_assoc($sqlRev);
//d($sqlROW); exit;								
while($sqlROW=mysqli_fetch_array($sqlRev))
{
	//echo $sqlROW['client_name'];
	...
	if($sqlROW['fb_id']!='') 
	{
		...
		//echo "UPDATE cashflow2024 SET fb_spent='".implode(',',$fbSpent)."' WHERE tbl_id=".$sqlROW['tbl_id'].""; 
		if(!isset($_GET['ajax'])) { 
			mysqli_query($conn, "UPDATE cashflow2024 SET fb_spent='".implode(',',$fbSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error()); //exit;
		}
	}
	//exit;
	//Google
	if($sqlROW['g_id']!='') {
		...
		if(!isset($_GET['ajax'])) {
			mysqli_query($conn, "UPDATE cashflow2024 SET g_spent='".implode(',',$gSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());	
		}
	}
	...
	if($sqlROW['in_id']!='') { 
		...
		if(!isset($_GET['ajax'])) {
			mysqli_query($conn, "UPDATE cashflow2024 SET in_spent='".implode(',',$inSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());		
		}
	}
	//exit;
	//Taboola
	if($sqlROW['ta_id']!='' && $taboola_tok!='') {
		...
		if(!isset($_GET['ajax'])) {
			mysqli_query($conn, "UPDATE cashflow2024 SET ta_spent='".implode(',',$taSpent)."' WHERE tbl_id=".$sqlROW['tbl_id']."") or die(mysqli_error());	
		}
...
$sqlRev=mysqli_query($conn, "SELECT c.*, cp.amount, cp.date FROM cashflow2024 as c, cashflow_payments2024 as cp WHERE c.tbl_id=cp.cashflow_id AND c.uid='2' AND c.delete_status=0 order by c.tbl_id asc");
...
{ 
	...
	if($sqlROW["date"]!='' && !isset($_GET['refresh'])) 
	{
		...
		foreach($last_paid_dt as $k => $date){
		  ...
		  //echo $last_paid[$k].' - '.$mostRecent;  exit;
			$last_paid_per = $last_paid = 0;
			if(isset($last_paid[$k])){
				if($last_paid[$k]!=0) { $last_paid_per = round(@($tot_bal / $last_paid[$k]) * 100); }
		...
		if($tot_bal > 0 && $last_paid_per<25 && $sqlROW["daily_budget"]!='') {
			$em_type = 2;
			$client = $sqlROW["client_name"];
			if($sqlROW["daily_budget"]!=0) { $days_left = round(@($tot_bal / $sqlROW["daily_budget"])); }
			//$ad_bal = moneyFormatIndia($tot_bal);
			$to_address = "accounts@bytindia.com,faheem@bytindia.com,".$sqlROW["email"];
			//include 'email/mail-cashflow-client.php';
		}
	...
	$tbl .='
		<tr>
			...
			<td style="text-align: right;">'.moneyFormatIndia($sqlROW["tot_paid"]).'</td>
			<td style="text-align: right;">'.moneyFormatIndia($tot_spend).'</td>
			<td style="background-color:'.$bgcolor.';text-align: right;"><b>'.moneyFormatIndia($tot_bal).'</b></td>
		</tr>
	';
	if($sno==($totRows)){ 
		$tbl .='
			<tr>
				<td>-</td>
				<td><b>Total</b></td>
				<td style="text-align: right;"><b>'.moneyFormatIndia($grandRecived).'</b></td>
				<td style="text-align: right;"><b>'.moneyFormatIndia($grandSpent).'</b></td>
				<td style="background-color:'.$bgcolor.';text-align: right;"><b>'.moneyFormatIndia($grandBalance).'</b></td>
			</tr>
		';
	}
...
if(isset($_GET['refresh'])) {
	echo 'success';
	echo "<script>window.location = 'cashflow-2024.php';</script>"; 
	exit();
} else {
