...
foreach($fb_acc_ids as $k => $val) {
    $url_1 = "https://graph.facebook.com/".$api_ver."/act_".$val."/insights?level=account&fields=objective,spend,actions&time_range[since]=".date("Y-m-d", strtotime('0 days'))."&time_range[until]=".date("Y-m-d", strtotime('0 days'))."&access_token=".$access_token."&limit=250";
    $req_1 = file_get_contents_curl($url_1);
	$res_1 = json_decode($req_1, true);  
    $resData[$val][1] = $res_1;

    $url_2 = "https://graph.facebook.com/".$api_ver."/act_".$val."/insights?level=account&fields=objective,spend,actions&time_range[since]=".date("Y-m-d", strtotime('-1 days'))."&time_range[until]=".date("Y-m-d", strtotime('-1 days'))."&access_token=".$access_token."&limit=250";
    $req_2 = file_get_contents_curl($url_2);
	$res_2 = json_decode($req_2, true);  
    $resData[$val][2] = $res_2;

    $url_3 = "https://graph.facebook.com/".$api_ver."/act_".$val."/insights?level=account&fields=objective,spend,actions&time_range[since]=".date("Y-m-d", strtotime('-30 days'))."&time_range[until]=".date("Y-m-d", strtotime('-1 days'))."&access_token=".$access_token."&limit=250";
