    ...
    <tbody>
            ...
            <?php foreach($ad_acc as $k => $val)  { 
                ...
                foreach($tbl_cols as $k1 => $val2) { 
                    $v = explode('_#_',$val2); 
                    $lead = 0; $per = $max_lead = 0; $max_css ='';
                    if(isset($lead_data[$k][$val2])) { $lead = round(array_sum($lead_data[$k][$val2])); $max_lead = max($maxLead[$k]); }
                    if($lead==0) { $lead_per = '-';  } else { $per = round((($lead/array_sum($lead_tot[$k]))*100),1); $lead_per = $lead.' <span class="font-italic percent">'.$per.' %</span>'; }
                    if($max_lead!=0 && $max_lead==$lead) { $max_css = 'bg_green"'; }
                    if($per>15 && (ucwords(str_replace('_',' ',$v[1]))=='Instream Video' || ucwords(str_replace('_',' ',$v[1]))=='Video Feeds')) { $max_css = 'bg_red"'; }
...
$('#reportrange span').html(d1 + ' - ' + d2);
        ...
        function(start, end) {
            if(start.format('DD/MM/YYYY')==defSt) {
                ...
                $('#stDt_upd').val('');
                $('#enDt_upd').val('');
                window.location = 'loading.php?pg=placement-casa.php';
            
            } else {
                //alert(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                $('#reportrange span').html(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                window.location = 'loading.php?pg=placement-casa.php&st='+start.format('DD/MM/YYYY')+'&en='+end.format('DD/MM/YYYY');
                startDate = start;
                endDate = end;   
