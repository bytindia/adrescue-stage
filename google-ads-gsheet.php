...
class GetCampaigns
{
    ...
    public static function main($conn,$g_ref_tok,$g_mcc,$adAccId, $st, $en,$projN)
    {
		//$adAccId = '2470684564';
		 ...
		 */
		public static function runExample(GoogleAdsClient $googleAdsClient, int $customerId, $st, $en, $projN)
		{
			...
			metrics.conversions,
			segments.date
			FROM campaign WHERE campaign.name LIKE '%".$projN."%' AND segments.date >= '".$st."' AND segments.date <= '".$en."' ORDER BY campaign.id"; 
			...
			foreach ($stream->iterateAllElements() as $googleAdsRow) {
				...
				//$gSpent[] = round($cost);
				//echo '<br>';
                $campaignData[] = array('campaign'=>$name, 'id'=>$id, 'cost'=>$cost, 'conversions'=>$conv, 'clicks'=>$clicks, 'ctr'=>$ctr, 'impressions'=>$impressions, 'date'=>$date);
			}
			/*echo '<br>';
			print_r($cost);
			echo '<br>';
