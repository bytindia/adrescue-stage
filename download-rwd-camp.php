...
class ParallelReportDownload
{
    ...
    public static function runExample(
        ...
        $selector->setFields(
            ['CampaignName', 'AverageCpc', 'CampaignId', 'Impressions', 'Clicks', 'Cost', 'AverageCost', 'AverageCpm', 'CostPerConversion', 'Conversions']
        );
    ...
    public static function main($ClientIds, $fromDt, $enDt, $g_mcc, $g_refresh_token, $server_path, $key)
    {
...
if($filter=='no') {
    foreach($g_acc_ids as $gR)
    {
        $fromDt = date("Ymd", strtotime('0 days'));
        $enDt = date('Ymd', strtotime('0 days')); 
        ParallelReportDownload::main(array($gR), $fromDt, $enDt, $g_mcc, $g_refresh_token, $server_path, 1);
    }
    foreach($g_acc_ids as $gR)
    {
        $fromDt = date("Ymd", strtotime('-1 days'));
        $enDt = date('Ymd', strtotime('-1 days')); 
        ParallelReportDownload::main(array($gR), $fromDt, $enDt, $g_mcc, $g_refresh_token, $server_path, 2);
    }
}
foreach($g_acc_ids as $gR)
{
    $fromDt = date("Ymd", strtotime($dtRange1));
	$enDt = date('Ymd', strtotime($dtRange2)); 
	ParallelReportDownload::main(array($gR), $fromDt, $enDt, $g_mcc, $g_refresh_token, $server_path, 3);
}
