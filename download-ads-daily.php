...
foreach ($weeks as $key => $value)  {
	...
	$gIds = array();
	//d($accIds); exit;	
	$sqlD = mysqli_query($conn, "select acc_id, report_1 from ads_daily WHERE uid='2' AND acc_type='G' AND acc_id IN (".implode(',',$accIds).") AND st_dt='".$weeks[$key][0]."' AND en_dt='".$weeks[$key][1]."'");
	while($Rdata=mysqli_fetch_array($sqlD)) {
			$gIds[] = $Rdata['acc_id'];
			$arr_goo[$Rdata['acc_id']] = unserialize($Rdata['report_1']);
	}
