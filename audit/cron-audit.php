...
function AdSet($url, $conn, $accId, $rep_id, $marks) { 
	...
	foreach ($fb_response->data as $key => $res) {
			...
			$AdsetQ = mysqli_query($conn, "select * from audit_adset WHERE adset_id='".$res->id."'");						
...
function AdSetInsights($url, $conn, $accId, $obj_arr, $rep_id, $marks) { 
	...
	foreach ($fb_response['data'] as $key => $res) {
			...
			$AdInQ = mysqli_query($conn, "select * from audit_ad WHERE ad_id='".$res['id']."'");						
	...
	if(isset($fb_response['paging']['next'])) {
		...
		$sql_1 = mysqli_query($conn, "SELECT ad_type, SUM(spend) AS spend, SUM(lead) as lead, (spend/lead) as per FROM `audit_ad` where obj_type='LEAD_GENERATION' GROUP BY ad_type");
