...
//d($_SESSION['metrics']); exit;

$metrics = $_SESSION['metrics'];

$tot_bal = $tot = 0;
$sqlRev=mysqli_query($conn, "SELECT name, account_id FROM adAccounts WHERE uid='2' and account_status=1");
...
{ 
    $adAcc[$sqlROW['account_id']] = mysqli_real_escape_string($conn, preg_replace('/[^A-Za-z0-9\-]/', '', $sqlROW['name']));
}
$sel_acc = explode(',', $_SESSION['ad_acc']);
foreach($sel_acc as $k => $val) 
{ 
    $ad_acc[$val] =  $adAcc[$val];
}
//d($ad_acc); exit;
 //$ad_acc = array( 807092046796106=>'Flagship',1744658879060018=>'Boulevard');
  ...
  { 
    //$val = $acc_id;
     $url = "https://graph.facebook.com/".$api_ver."/act_".$k."/insights?level=campaign&fields=".$metrics.",actions,objective&time_range[since]=".date("Y-m-d", strtotime($dtRange1))."&time_range[until]=".date("Y-m-d", strtotime($dtRange2))."&access_token=".$access_token."&limit=750";
...
$('#reportrange span').html(d1 + ' - ' + d2);
        ...
        function(start, end) {
            if(start.format('DD/MM/YYYY')==defSt) {
                ...
                $('#stDt_upd').val('');
                $('#enDt_upd').val('');
                window.location = 'loading.php?pg=custom-report.php';
            
            } else {
                //alert(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                $('#reportrange span').html(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                window.location = 'loading.php?pg=custom-report.php&st='+start.format('DD/MM/YYYY')+'&en='+end.format('DD/MM/YYYY');
                startDate = start;
                endDate = end;   
