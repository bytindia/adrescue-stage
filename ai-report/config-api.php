...
$pgQuery = "
    SELECT 
        ...
        l.pg_id,
        p.pg_token
    FROM leads_acc as l, pages as p
    WHERE l.pg_id = p.pg_id
      AND l.uid = ?
      AND l.delete_status=0
";

$pgStmt = $conn->prepare($pgQuery);
$pgStmt->bind_param("i", $uid);
$pgStmt->execute();
$pgResult = $pgStmt->get_result();

$pgcounts = [];

while ($row = $pgResult->fetch_assoc()) {
    $pgcounts[] = [
        "client_name"  => $row['client_name'],
        "page"     => $row['pg_name'],
        "page_id"     => $row['pg_id'],
        "page_token" => $row['pg_token']
    ];
}
