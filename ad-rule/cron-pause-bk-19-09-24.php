...
{       
        ...
        if($ad_acc_type[$k]==2) { //Coaching
            $qryparam .= ",action_values&filtering=[{'field':'objective','operator':'IN','value':[".$cam_obj[$ad_acc_type[$k]]."]},{'field':'action_type','operator':'IN','value':['offsite_conversion.fb_pixel_purchase','purchase'"; 
        ...
        echo $url = "https://graph.facebook.com/".$api_ver."/act_".$val."/insights?level=ad&fields=ad_id,adset_id,campaign_id,campaign_name,spend,objective,actions".$qryparam."&time_range[since]=".$rep_dt[$k][1]."&time_range[until]=".$rep_dt[$k][0]."&access_token=".$access_token."&limit=750"; //exit;
        ...
        {
            //$act_ads_camp_ids = $act_ads_adset_ids = array();  

            foreach($fb_data[$k] as $k_fb => $v_fb) {
                $lead = $cpl = $pur_roas = 0; 
                $cust_val = array();

                if($ad_acc_type[$k]==2) {
                    if(count($cust_conv_ids_db[$k])==0 && isset($v_fb['action_values'])) { $cust_val['purchase'] = LeadGen($v_fb['action_values'], 'purchase'); }
                    if(count($cust_conv_ids_db[$k])>0 && isset($v_fb['action_values'])) { $cust_val['cust_conv'] = LeadGen($v_fb['action_values'], $cust_conv_ids_db[$k][0]); }
                }
                if($ad_acc_type[$k]==4) {
                ...
                $camp_data_ri[$k][$v_fb['campaign_id']][] = array('spend'=> $v_fb['spend'], 'lead'=> $lead, 'camp_name'=>$v_fb['objective'], 'obj'=>$v_fb['objective'], 'cust_val'=>$cust_val, 'pur_roas'=>$pur_roas);
                $adset_data_ri[$k][$v_fb['adset_id']][] = array('spend'=>$v_fb['spend'], 'lead'=> $lead, 'camp_name'=>$v_fb['campaign_name'], 'obj'=>$v_fb['objective'], 'cust_val'=>$cust_val, 'pur_roas'=>$pur_roas);
                $ad_data_ri[$k][$v_fb['ad_id']][] = array('spend'=>$v_fb['spend'], 'lead'=> $lead, 'camp_name'=>$v_fb['campaign_name'], 'obj'=>$v_fb['objective'], 'cust_val'=>$cust_val, 'pur_roas'=>$pur_roas);
            }
