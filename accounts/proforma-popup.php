<?php
// Proforma Invoice Sent Details Popup - Loaded via AJAX
session_start();
...
// Fetch proforma invoice data for current month
$currentYear = date('Y');
$currentMonth = date('m');

$where = ["i.approved='yes'", "i.inv_ty='pi'"];
...
<div class="table-responsive">
    <table id="proformaDataTable" class="table table-striped table-bordered table-hover" style="width:100%">
        <thead>
            <tr>
                <th>Inv. Date</th>
                <th>Client</th>
...
if ($.fn.DataTable && $('#proformaDataTable').length) {
    // Destroy existing DataTable if it exists
    if ($.fn.DataTable.isDataTable('#proformaDataTable')) {
        $('#proformaDataTable').DataTable().destroy();
    }
    $('#proformaDataTable').DataTable({
