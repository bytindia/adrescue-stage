 ...
 *
 * Although the example's name is `ParallelReportDownload2`, it doesn't download
...
class ParallelReportDownload2
{
    ...
    public static function runExample(
        ...
        $selector->setFields(
            ['AccountDescriptiveName', 'Cost', 'AverageCost', 'AverageCpm', 'CostPerConversion', 'Ctr', 'Conversions']
        );
...
foreach ($weeks2 as $key => $value)  {
	
	$fromDt = date('Ymd', $weeks2[$key][0]);
	$enDt = date('Ymd', $weeks2[$key][1]);
	
	$gIds = array();
	//d($accIds); exit;	
	$sqlD = mysqli_query($conn, "select acc_id, report_1_30d from ads_daily WHERE uid='2' AND acc_type='G' AND acc_id IN (".implode(',',$accIds).") AND st_dt='".$weeks[$key][0]."' AND en_dt='".$weeks[$key][1]."'");
	while($Rdata=mysqli_fetch_array($sqlD)) {
			$gIds[] = $Rdata['acc_id'];
			$arr_goo2[$Rdata['acc_id']] = unserialize($Rdata['report_1_30d']);
	}
	
	$resIds = array_diff($accIds, $gIds);  
	//d($gIds); d($arr_goo2); exit;
	if(count($resIds)>0) 
	{
		ParallelReportDownload2::main($resIds, $fromDt, $enDt, $key, $g_mcc, $g_refresh_token, $server_path);
	}
	//exit;
}
//ParallelReportDownload2::main($accIds, $accNames, $fromDt, $enDt, $key, $g_mcc, $g_refresh_token);
//ParallelReportDownload2::main($accIds, $accNames, $fromDt, $enDt);
