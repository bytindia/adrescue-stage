...
if (isset($_GET['del'])) {
  mysqli_query($conn, "UPDATE cards SET delete_status='1' where tbl_id=" . $_GET['del'] . "");
...
$statement = " cards WHERE uid='" . $_SESSION['uid'] . "' AND delete_status=0";
        ...
        <div class="row">
          <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                ...
                <ul class="nav navbar-right panel_toolbox">
                  ...
                  <li>
                    <div class="btn-group  btn-group-sm">
                      <a href="loading.php?pg=cards-add.php" class="btn btn-success btn-sm">Add New Card</a>
                    </div>
                  </li>
                  <li> &nbsp;</li>
                  <li>
                    <div class="btn-group  btn-group-sm">
                      <a href="loading.php?pg=cron-cashflow-2025.php?refresh=1" class="btn btn-warning btn-sm" onclick="return confirm('Are you sure you want to Email Cashflow report?');"><i class="fa fa-refresh"></i> Refresh All</a>
                    </div>
                  </li>
                  <li> &nbsp;</li>
                  <li>
              ...
              <div class="x_content">
                <?php //echo "SELECT * FROM ".$statement." order by tbl_id desc"; 
                ...
                <form method="post" action="">
                  <table id="datatable" class="table table-hover table-striped table-bordered">
                    ...
                    <tbody>
                      ...
                      while ($sqlROW = mysqli_fetch_array($sqlRev)) {
                        ...
                        if ($sqlROW["fb_spent"] != '') {
                          $fb_spent = explode(',', $sqlROW["fb_spent"]);
                          $fb_spent = array_sum(array_filter($fb_spent));
                        }
                        if ($sqlROW["g_spent"] != '') {
                          $g_spent = explode(',', $sqlROW["g_spent"]);
                          $g_spent = array_sum(array_filter($g_spent));
                        }
                        if ($sqlROW["in_spent"] != '') {
                          $in_spent = explode(',', $sqlROW["in_spent"]);
                          $in_spent = array_sum(array_filter($in_spent));
                        }
                        if ($sqlROW["ta_spent"] != '') {
                          $ta_spent = explode(',', $sqlROW["ta_spent"]);
                      ...
                      ?>
                        <tr>
                          <td><?php echo $i; ?></td>
                          <td><a onClick="onAjax(<?php echo $sqlROW["tbl_id"]; ?>, 1, '<?php echo $sqlROW["card_name"]; ?>');" data-id="1" data-toggle="modal" class="blue modal-cl" data-target=".bs-example-modal-lg"><?php echo $sqlROW["card_name"]; ?></a><!-- <i class="fa fa-facebook"></i> <i class="fa fa-google"></i> <i class="fa fa-linkedin"></i> <i class="fa fa-taboola"></i>  --></td>
                          <td><a onClick="onAjax(<?php echo $sqlROW["tbl_id"]; ?>, 2, '<?php echo $sqlROW["card_name"]; ?> - Payment Received Detail');" data-id="1" data-toggle="modal" class="blue modal-cl" data-target=".bs-example-modal-lg"> <?php echo money_format('%.0n', $sqlROW["daily_budget"]); ?></a></td>
                          <td><a onClick="onAjax(<?php echo $sqlROW["tbl_id"]; ?>, 1, '<?php echo $sqlROW["card_name"]; ?> - Spent Detail');" data-id="1" data-toggle="modal" class="blue modal-cl" data-target=".bs-example-modal-lg"> <?php echo money_format('%.0n', $tot_spend); ?></a></td>
                          <td> <?php echo money_format('%.0n', $tot_bal); ?></td>
                          <td>
                            <a href="loading.php?pg=cron-cards.php?tbl_id=<?php echo $sqlROW["tbl_id"]; ?>&refresh=1" class="btn btn-warning btn-sm"><i class="fa fa-refresh"></i></a>
                            <a href="loading.php?pg=cards-add.php?id=<?php echo $sqlROW["tbl_id"]; ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i> </a>
                            <a href="loading.php?pg=cards.php?del=<?php echo $sqlROW["tbl_id"]; ?>" onclick="return confirm('Are you sure you want to delete this?');" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
                        ...
                        if ($i == ($totRows + 1)) { ?>
                          ...
                          <td><b>Total</b></td>
                          <td><b> <?php echo money_format('%.0n', $grandRecived); ?></b></td>
                  ...
                  <div id="pagDiv"><?php //echo pagination($statement,$per_page,$page,$url='?',''); 
                                    ?></div>
