...
function moneyFormatIndia($num) {
    ...
    return $neg.''.$thecash; // writes the final format where $currency is the currency symbol.
}
...
if(isset($_SESSION['st'])) { 
 ...
 //$ad_acc = array( 807092046796106=>'Flagship',1744658879060018=>'Boulevard');
  ...
  ?>
  <?php 
$filterDates = getDatesFromRange($dtRange1, $dtRange2);
//d($filterDates); //exit;
$bud_req = mysqli_query($conn, "SELECT budget, client_name, date, leads, acc_ids from budget_casa where tbl_id=1");
$bud_res = mysqli_fetch_assoc($bud_req);

$bud_amount = unserialize($bud_res['budget']);
$bud_date = unserialize($bud_res['date']);
$bud_lead = unserialize($bud_res['leads']);
$bud_acc = unserialize($bud_res['acc_ids']);
global $budSpend , $budLead;

foreach($bud_acc as $key => $val) {
    $newDate = date("dmY", strtotime($bud_date[$key]));  
    $budDates[$val][] = $newDate;
    $budSpend[$val][] = $bud_amount[$key];
    $budLead[$val][] = $bud_lead[$key];
}
//d($budData); exit;
//d($budDates);
...
<table id="datatable" class="table table-hover table-striped table-bordered datatable" data-ordering="true">
    ...
    <tbody>
                ...
                <?php 
                function getBudgetRep($arr,$accId){
                    global $budSpend, $budLead;
                    $spend = $lead = 0;
                    foreach($arr as $k => $v) {
                        
                        $spend +=$budSpend[$accId][$k];
                        $lead +=$budLead[$accId][$k];
                    }
                    return array($spend, $lead);
                }
                $tot_spend = $tot_lead = $tot_cpl = $lead_ex_tot = 0;
                foreach($ad_acc as $k => $val)  { 
                    
                    $containsAllValues = '';
                    if(count($budDates[$k])>0){
                        $containsAllValues = !array_diff($filterDates, $budDates[$k]);
                    }
                    ...
                    $diffkeys   = array_intersect($budDates[$k], $filterDates);
                    $bud_est = $lead_est = $bud_bal = $lead_ex = 0;
                    if(count($diffkeys)>0){
                        $getBudgetRep = getBudgetRep($diffkeys,$k);
                        $bud_est = $getBudgetRep[0];
                        $lead_est = $getBudgetRep[1];
                        $bud_est_tot +=$bud_est;
                        $lead_est_tot +=$lead_est;
                        $bud_bal = $bud_est - $spend;
                    }
                    if($bud_bal>0) {
                        $lead_ex = $bud_bal / round(@($spend/$lead));
                    }
                    if(round($lead_ex)!=0 && round($lead)!=0) { $lead_ex_tot += $lead_ex; }
                ?>
                    <tr> 
                    <td><a href="loading.php?pg=placement-lg.php&act_id=<?php echo $k; ?>" target="_blank"><?php echo $val; ?></a></td>
                    <td><?php echo moneyFormatIndia($bud_est); ?></td>
                    <td><?php echo moneyFormatIndia($lead_est); ?></td>
                    <td><?php echo moneyFormatIndia($spend); ?></td>
                    <td><?php echo moneyFormatIndia($lead); ?></td>
                    <td><?php if(round($lead_ex)!=0 && round($lead)!=0) { echo moneyFormatIndia($lead_ex); } else { echo '-'; } ?></td>
                    <td><?php if(round($lead)!=0) { echo moneyFormatIndia(round(@($spend/$lead))); } else { echo '-'; } ?></td>
                    <td><?php echo moneyFormatIndia($bud_bal); ?></td>
                ...
                <tr> 
                    <td>Total</td>
                    <td><?php echo moneyFormatIndia($bud_est_tot); ?></td>
                    <td><?php echo moneyFormatIndia($lead_est_tot); ?></td>
                    <td><?php echo moneyFormatIndia($tot_spend); ?></td>
                    <td><?php echo moneyFormatIndia($tot_lead); ?></td>
                    <td><?php echo moneyFormatIndia($lead_ex_tot); ?></td>
                    <td><?php if(round($tot_lead)!=0) { echo moneyFormatIndia(round(@($tot_spend/$tot_lead))); } else { echo '-'; }?></td>
                    <td><?php echo moneyFormatIndia($bud_est_tot-$tot_spend); ?></td>
...
$('#reportrange span').html(d1 + ' - ' + d2);
        ...
        function(start, end) {
            if(start.format('DD/MM/YYYY')==defSt) {
                ...
                $('#stDt_upd').val('');
                $('#enDt_upd').val('');
                window.location = 'loading.php?pg=spend-casa2.php';
            
            } else {
                //alert(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                $('#reportrange span').html(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                window.location = 'loading.php?pg=spend-casa2.php&st='+start.format('DD/MM/YYYY')+'&en='+end.format('DD/MM/YYYY');
                startDate = start;
                endDate = end;   
