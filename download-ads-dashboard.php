...
session_start();
require $server_path . '/g-vendor/autoload.php';
...
class ParallelReportDownload
{
    ...
    public static function runExample(
        ...
        $selector->setFields(           
			 ['CampaignName', 'AverageCpc', 'Impressions', 'Clicks', 'Cost', 'AverageCost', 'AverageCpm', 'CostPerConversion', 'Ctr', 'Conversions','BounceRate', 'AverageTimeOnSite', 'CampaignStatus']
        );
		//$selector->dateRange = new DateRange('20140101', '20140110');
		$selector->setDateRange(new DateRange($fromDt, $enDt));
        // Create report definition.
        $reportDefinition = new ReportDefinition();
        $reportDefinition->setSelector($selector);
        $reportDefinition->setReportName('Custom CAMPAIGN_PERFORMANCE_REPORT');
     ...
     * create new session.
     */
    private static function getAllManagedCustomerIds(AdWordsServices $adWordsServices, AdWordsSession $session) 
	{
    ...
    public static function main($ClientIds, $fromDt, $enDt, $key, $g_mcc, $g_refresh_token, $server_path)
    {
...
foreach ($st_dates as $key => $value)  {
	...
	$fromDt = date('Ymd', strtotime($st_dates[$key]));
	$enDt = date('Ymd', strtotime($en_dates[$key]));
	
	$gIds = array();
	//d($accIds); exit;	
	$sqlD = mysqli_query($conn, "select acc_id, report_1 from ads_weekly WHERE uid='2' AND acc_type='G' AND acc_id IN (".implode(',',$accIds).") AND st_dt='".strtotime($st_dates[$key])."' AND en_dt='".strtotime($en_dates[$key])."'");
	while($Rdata=mysqli_fetch_array($sqlD)) {
			$gIds[] = $Rdata['acc_id'];
			$arr_goo[$Rdata['acc_id']][$key] = unserialize($Rdata['report_1']);
	}
	
	$resIds = array_diff($accIds, $gIds);  
	//d($gIds); d($arr_goo); exit;
	if(count($resIds)>0) 
	{
		ParallelReportDownload::main($resIds, $fromDt, $enDt, $key, $g_mcc, $g_refresh_token, $server_path);
	}
	//exit;
}
//ParallelReportDownload::main($accIds, $accNames, $fromDt, $enDt, $key, $g_mcc, $g_refresh_token);
//ParallelReportDownload::main($accIds, $accNames, $fromDt, $enDt);
