...
class GetAccountHierarchy
{
    ...
    private const MANAGER_CUSTOMER_ID = 7794369179;
    ...
    private const LOGIN_CUSTOMER_ID = 7794369179;
    ...
    public static function main($conn, $g_ref_tok,$g_mcc)
    {
        ...
        $managerCustomerId = $loginCustomerId = $g_mcc;
        ...
        try {
            return self::runExample($googleAdsClient, $managerCustomerId, $loginCustomerId, $conn, $g_ref_tok,$g_mcc);
    ...
    public static function runExample(
        ...
        foreach ($rootCustomerIds as $rootCustomerId) {
            $customerClientToHierarchy =
                self::createCustomerClientToHierarchy($loginCustomerId, $rootCustomerId,$conn, $g_ref_tok,
                $g_mcc);
    ...
    private static function createCustomerClientToHierarchy(
        ?int $loginCustomerId,
        int $rootCustomerId, $conn,  $g_ref_tok,
        $g_mcc
    ): ?array {
    ...
    private static function printAccountHierarchy(
       ...
       //echo $customerId.' - '.$customerClient->getDescriptiveName().' - '.$customerClient->getCurrencyCode().' - '.$customerClient->getTimeZone().'<br>';
        $cirRes = mysqli_query($conn, "select * from gaccounts WHERE account_id='".$customerId."' AND uid='".$_SESSION['uid']."'");						
			
					//echo "select * from circuit where (editID='".$_POST['id']."' || connID='".$_POST['id']."') AND (editID='".$value."' || connID='".$value."')";
					if(mysqli_num_rows($cirRes)==0) {
                            $cirSql = "INSERT INTO gaccounts (uid, name, account_id, currency, account_status, created) VALUES ('".$_SESSION['uid']."', '".mysqli_real_escape_string($conn, $customerClient->getDescriptiveName())."', '".mysqli_real_escape_string($conn, $customerId)."', '".mysqli_real_escape_string($conn, $customerClient->getCurrencyCode())."', '".mysqli_real_escape_string($conn, $customerClient->getStatus())."', now());"; 
                            //echo '<br>';
							mysqli_query($conn, $cirSql) or die(mysqli_error()); 
						} else {
							   //$cirSql = "UPDATE gaccounts SET name='".mysqli_real_escape_string($conn, $customerClient->getDescriptiveName())."', updated=now() WHERE account_id='".$customerId."' AND g_id='".$_SESSION['g_id']."' AND uid='".$_SESSION['uid']."'";
                               $cirSql = "UPDATE gaccounts SET name='".mysqli_real_escape_string($conn, $customerClient->getDescriptiveName())."', account_status='".mysqli_real_escape_string($conn, $customerClient->getStatus())."', updated=now() WHERE account_id='".$customerId."' AND uid='".$_SESSION['uid']."'";
                         // echo '<br>';
							mysqli_query($conn, $cirSql) or die(mysqli_error()); 
					}
        ...
        if (array_key_exists($customerId, $customerIdsToChildAccounts)) {
            foreach ($customerIdsToChildAccounts[$customerId] as $childAccount) {
                self::printAccountHierarchy($childAccount, $customerIdsToChildAccounts, $depth + 1, $conn);
            }
        }
    }
}
...
GetAccountHierarchy::main($conn, $_SESSION['g_refresh_token'], $_SESSION['g_mcc']);
//exit;
if($redirect == 'yes') {
