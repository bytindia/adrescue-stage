...
//Get Campaign List
/*
$url = "https://graph.facebook.com/".$api_ver."/act_".$acc_id."/insights?level=ad&fields=spend,objective,actions&time_range[since]=".date("Y-m-d", strtotime($stDt))."&time_range[until]=".date("Y-m-d", strtotime($enDt))."&access_token=".$access_token."&limit=750";
$req = file_get_contents_curl($url);
$res = json_decode($req, true);  */

//Get Campaign List
$url = "https://graph.facebook.com/".$api_ver."/act_".$acc_id."/ads?fields=spend,adcreatives{adlabels,object_type,image_url,thumbnail_url}&time_range[since]=".date("Y-m-d", strtotime($stDt))."&time_range[until]=".date("Y-m-d", strtotime($enDt))."&access_token=".$access_token."&limit=5";
$req = file_get_contents_curl($url);
$res = json_decode($req, true); 
