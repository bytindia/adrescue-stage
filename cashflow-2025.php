...
if(isset($_GET['del'])) {
	mysqli_query($conn, "UPDATE cashflow2025 SET delete_status='1' where tbl_id=".$_GET['del']."");
	$_SESSION['suc'] = 'Successfully Deleted!';	
	echo "<script>window.location = 'cashflow-2025.php';</script>";
	exit();
}
...
$statement = " cashflow2025 WHERE uid='".$_SESSION['uid']."' AND delete_status=0";
                    ...
                    <ul class="nav navbar-right panel_toolbox">
                      <li>
                        <div class="btn-group btn-group-sm" role="group">
                          <a href="loading.php?pg=cashflow-add-2025.php" class="btn btn-primary btn-sm">Add Accounts</a>
                          <a href="loading.php?pg=cron-cashflow-2025.php?refresh=1" class="btn btn-primary btn-sm" onclick="return confirm('Are you sure you want to Email Cashflow report?');"><i class="fa fa-refresh"></i> Refresh All</a>
                    ...
                    $grandRecived = $grandSpent = $grandBalance = 0;
                      ...
                      $grandBalance = $grandBalance + $tot_bal;
                                        ...
                                        <tr>                                        	
                                            ...
                                            <td>
                                              <div class="btn-group btn-group-sm" role="group">
                                                <a href="loading.php?pg=cron-cashflow-2025.php?tbl_id=<?php echo $sqlROW["tbl_id"]; ?>&refresh=1" class="btn btn-warning btn-sm" title="Refresh"><i class="fa fa-refresh"></i></a>
                                                <a href="loading.php?pg=cashflow-add-2025.php?id=<?php echo $sqlROW["tbl_id"]; ?>" class="btn btn-primary btn-sm" title="Edit"><i class="fa fa-pencil"></i></a>
                                                <a href="loading.php?pg=cashflow-2025.php?del=<?php echo $sqlROW["tbl_id"]; ?>" onclick="return confirm('Are you sure you want to delete this?');" class="btn btn-danger btn-sm" title="Delete"><i class="fa fa-trash-o"></i></a>
                                              </div>
                                            </td>                              	
                                        </tr>  
...
var defSt = '01/01/2020';
var defEnd = '01/01/2026';
        ...
        function(start, end) {
            if(start.format('DD/MM/YYYY')==defSt) {
                ...
                $('#stDt_upd').val('');
                $('#enDt_upd').val('');
                window.location = 'cashflow-2025.php';
            
            } else {
                //alert(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                $('#reportrange span').html(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                window.location = 'cashflow-2025.php?st='+start.format('DD/MM/YYYY')+'&en='+end.format('DD/MM/YYYY');
                startDate = start;
                endDate = end;   
...
function onAjax(id,ty,clName) {
          ...
          if(isset($_GET['st']) && $_GET['st']!=''){
          ?>
            var page = 'cron-cashflow-2025.php?tbl_id='+id+'&refresh=1&ajax=1&<?php echo $dt_q; ?>';
          ...
          ?>
            var page = 'ajax-cashflow-2025.php';
            var dt1 ='';
          <?php
          }
          ?>
