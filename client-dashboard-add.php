...
Auth();
$pgHeadline = 'Add Account - Client Dashboard';
$pgID = 8;
$err =''; 
...
if(isset($_POST['submit'])){
	//print_r($_POST); exit;
	if($_POST['fb_acc']!='' || $_POST['fb_acc']!='' || $_POST['in_acc']!='') 
	{
		...
		if(isset($_GET['id'])) {
			$cirSql = "UPDATE client_dashboard SET client_name='".mysqli_real_escape_string($conn, $_POST['client'])."', fb_acc='".mysqli_real_escape_string($conn, $_POST['fb_acc'])."',  g_acc='".mysqli_real_escape_string($conn, $_POST['g_acc'])."',  in_acc='".mysqli_real_escape_string($conn, $_POST['in_acc'])."', weekly='".mysqli_real_escape_string($conn, $_POST['weekly'])."', monthly='".mysqli_real_escape_string($conn, $_POST['monthly'])."', email_ids='".mysqli_real_escape_string($conn, $_POST['email_ids'])."', updated=now() WHERE tbl_id=".$_GET['id']."";
			mysqli_query($conn, $cirSql) or die(mysqli_error()); 
			$lastId = $_GET['id'];
		} else {
			 $cirSql = "INSERT INTO client_dashboard (uid, fb_id, client_name, fb_acc, g_acc, in_acc, weekly, monthly, email_ids, created, updated) VALUES ('".$_SESSION['uid']."', '".$_SESSION['fb_id']."', '".mysqli_real_escape_string($conn, $_POST['client'])."', '".mysqli_real_escape_string($conn, $_POST['fb_acc'])."', '".mysqli_real_escape_string($conn, $_POST['g_acc'])."', '".mysqli_real_escape_string($conn, $_POST['in_acc'])."', '".mysqli_real_escape_string($conn, $_POST['weekly'])."', '".mysqli_real_escape_string($conn, $_POST['monthly'])."', '".mysqli_real_escape_string($conn, $_POST['email_ids'])."', now(), now());"; 
			 mysqli_query($conn, $cirSql) or die(mysqli_error());
			 $lastId = mysqli_insert_id($conn);
		} 
		
		$today =  date('Y-m-d');
		$last90 =  date('Y-m-d', strtotime("-30 days"));
		
		//FACEBOOK ADS Report
		if(isset($_POST['fb_acc']) && trim($_POST['fb_acc'])!='') 
		{			
			$request_url = 'https://graph.facebook.com/'.$api_ver.'/act_'.$_POST['fb_acc'].'/insights?level=campaign&fields=objective,campaign_id,campaign_name,reach,spend,actions,ctr,cpc,cpm,impressions,clicks,outbound_clicks&date_preset=last_90d&time_increment=1&limit=300&access_token='.$access_token;
			$fbData = FB_Report($request_url, $_POST['fb_acc'], $_SESSION['uid'], $conn);
			//d($fbData); exit;
		}
		
		//Google ADS Report
		if(isset($_POST['g_acc']) && trim($_POST['g_acc'])!='') 
		{
			$accIds = array($_POST['g_acc']);
			include 'download-campaign-dashboard.php';	
			$gData = G_Report(''.$server_path.'ads-perform/dash_'.trim($_POST['g_acc']).'.csv', trim($_POST['g_acc']), $_SESSION['uid'], $conn);	//getCSVdata($csvFile, $gId, $conn)
			//d($gData); exit;
		}
		
		//LinkedIn ADS Report
		if(isset($_POST['in_acc']) && trim($_POST['in_acc'])!='') 
		{
			$uId = $_SESSION['uid'];
			$in_acc = $_POST['in_acc'];
		...
		echo "<script>window.location = 'client-dashboard-add.php?id=".$lastId."';</script>";
		exit();
	} 
}
?>
                  ...
                  <div class="x_content">
                  		<?php 
									...
									if(isset($_GET['id'])) {
										...
										$sqlD=mysqli_query($conn, "SELECT client_name, fb_acc,  g_acc, in_acc, email_notif, email_ids, monthly, weekly FROM client_dashboard WHERE tbl_id='".$_GET['id']."'");
                                    ...
                                    <div class="container-fluid">
                                      <div class="row">		
                                                ...
                                                <div class="col-md-8">     
                                                    ...
                                                    <select name="in_acc" id="in_acc" class="form-control">
                                                    	<option value="">Select LinkedIn Ad Account</option>
                                                    	<?php while($sqlROW3=mysqli_fetch_array($sqlRev3)) { ?>
                                                    	<option value="<?php echo $sqlROW3["account_id"]; ?>" <?php if(isset($editData['in_acc']) && $editData['in_acc']==$sqlROW3["account_id"]) { echo "selected='selected'"; } ?>><?php echo $sqlROW3["name"]; ?></option>
                                                        <?php } ?>
                                                    </select>
