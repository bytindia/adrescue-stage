...
Auth();
$pgHeadline = 'Add Account - Ads Performance';
$pgID = 8;
$err =''; 
...
if(isset($_POST['submit'])){
	...
	if($_POST['fb_acc']!='' || $_POST['fb_acc']!='') {
		...
		if(isset($_GET['id'])) {
			$cirSql = "UPDATE ads_perform SET client_name='".mysqli_real_escape_string($conn, $_POST['client'])."', fb_acc='".mysqli_real_escape_string($conn, $_POST['fb_acc'])."',  g_acc='".mysqli_real_escape_string($conn, $_POST['g_acc'])."', email_notif='".mysqli_real_escape_string($conn, $_POST['email_notif'])."', email_ids='".mysqli_real_escape_string($conn, $_POST['email_ids'])."', updated=now() WHERE tbl_id=".$_GET['id']."";
			mysqli_query($conn, $cirSql) or die(mysqli_error()); 
			$lastId = $_GET['id'];
		} else {
			 $cirSql = "INSERT INTO ads_perform (uid, fb_id, client_name, fb_acc, g_acc, email_notif, email_ids, created, updated) VALUES ('".$_SESSION['uid']."', '".$_SESSION['fb_id']."', '".mysqli_real_escape_string($conn, $_POST['client'])."', '".mysqli_real_escape_string($conn, $_POST['fb_acc'])."', '".mysqli_real_escape_string($conn, $_POST['g_acc'])."', '".mysqli_real_escape_string($conn, $_POST['email_notif'])."', '".mysqli_real_escape_string($conn, $_POST['email_ids'])."', now(), now());"; 
			 mysqli_query($conn, $cirSql) or die(mysqli_error());
			 $lastId = mysqli_insert_id($conn);
		}
		$_SESSION['suc'] = 'Successfully Updated!';	
		echo "<script>window.location = 'ads-perform.php';</script>";
		exit();
	} 
}
?>
                  ...
                  <div class="x_content">
                  		<?php 
									...
									if(isset($_GET['id'])) {
										...
										$sqlD=mysqli_query($conn, "SELECT client_name, fb_acc,  g_acc, email_notif, email_ids FROM ads_perform WHERE tbl_id='".$_GET['id']."'");
