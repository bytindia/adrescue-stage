...
class GetAccountHierarchy
{
    ...
    private static function printAccountHierarchy(
        ...
        echo $customerId.' - '.$customerClient->getDescriptiveName().' - '.$customerClient->getCurrencyCode().' - '.$customerClient->getStatus().'<br>';
        //$cirRes = mysqli_query($conn, "select * from gaccounts WHERE account_id='".$customerId."' AND uid='".$_SESSION['uid']."'");						
				...
				/*	if(mysqli_num_rows($cirRes)==0) {
                         $cirSql = "INSERT INTO gaccounts (uid, name, account_id, currency, created) VALUES ('".$_SESSION['uid']."', '".mysqli_real_escape_string($conn, $customerClient->getDescriptiveName())."', '".mysqli_real_escape_string($conn, $customerId)."', '".mysqli_real_escape_string($conn, $customerClient->getCurrencyCode())."', now());"; 
                            //echo '<br>';
