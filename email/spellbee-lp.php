...
$source ='';
		...
		if($_POST['src']!='' && $_POST['src']!='/') { $source = $_POST['src']; }
		$campaign = $_POST['sub_src'];
		$project_id = 1;

		
		$sqlQuery = "INSERT INTO spellbee_lp (name, email, phone, source, campaign, created) VALUES ('".mysqli_real_escape_string($conn, $name)."', '".mysqli_real_escape_string($conn, $email)."', '".mysqli_real_escape_string($conn, $phone)."', '".mysqli_real_escape_string($conn, $source)."', '".mysqli_real_escape_string($conn, $campaign)."',  now());"; 
        ...
        if(count($lead)>0) {
            ...
            $leadV = [[mysqli_real_escape_string($conn, $name),$email,'p:'.$phone,mysqli_real_escape_string($conn, $source), mysqli_real_escape_string($conn, $campaign),date('d-m-Y, h:i a')]];
...
try {
	...
	$mail->CharSet = 'UTF-8';
    //Recipients
    $mail->setFrom($mail_from_info, 'SpellBee - Enquiry');
	...
	$to_address = "query@spellbeeinternational.com, bytramesh@gmail.com, rebeccaspellbee@gmail.com"; 
