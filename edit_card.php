...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("UPDATE cards_data SET card=?, card_limit=?, total_payable=?, available_limit=?, percentage=? WHERE id=?");
    $stmt->bind_param("siiidi", $card, $card_limit, $total_payable, $available_limit, $percentage, $id);
    $stmt->execute();

    header("Location: cards.php");
    exit();
} else {
    $res = $conn->query("SELECT * FROM cards_data WHERE id = $id");
            ...
            <div class="col-md-6">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white">
                        <h4 class="text-center">Edit Card: <?php echo $data['card']; ?></h4>
                    </div>
                    <div class="card-body">
                        <form method="post">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="card" name="card" class="form-control" value="<?php echo $data['card']; ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="card_limit" class="form-label">Card Limit:</label>
                                <input type="number" id="card_limit" name="card_limit" class="form-control" value="<?php echo $data['card_limit']; ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="total_payable" class="form-label">Total Payable:</label>
                                <input type="number" id="total_payable" name="total_payable" class="form-control" value="<?php echo $data['total_payable']; ?>" required>
                            </div><br><br>
                            <div class="d-grid">
