<?php
// Budget Remaining Details Popup - Loaded via AJAX
session_start();
...
// Process budget remaining data (bal)
$tableData = [];
foreach ($clientRows as $row) {
    $bal = isset($row['bal']) ? (float)str_replace([',', ' '], '', $row['bal']) : 0;
...
<div class="table-responsive">
    <table id="budgetRemainingDataTable" class="table table-striped table-bordered table-hover" style="width:100%">
        <thead>
            <tr>
                <th>Client</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right" data-order="<?php echo $item['balance']; ?>">₹<?php echo moneyFormatIndia((int)$item['balance']); ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
...
if ($.fn.DataTable && $('#budgetRemainingDataTable').length) {
    // Destroy existing DataTable if it exists
    if ($.fn.DataTable.isDataTable('#budgetRemainingDataTable')) {
        $('#budgetRemainingDataTable').DataTable().destroy();
    }
    $('#budgetRemainingDataTable').DataTable({
        "order": [[3, "desc"]], // Sort by Balance column descending
        "pageLength": 25,
