...
class ParallelReportDownload
{
    ...
    public static function runExample(
        ...
        $selector->setFields(
            ['CampaignId', 'CampaignName', 'Ctr', 'Impressions', 'Clicks', 'Cost', 'AverageCost', 'AverageCpc', 'AverageCpm', 'Criteria', 'AdGroupName', 'QualityScore','HistoricalQualityScore', 'CostPerConversion']
        );
        ...
        $reportDefinition->setReportName('Custom KEYWORDS_PERFORMANCE_REPORT');
    ...
    public static function main($ClientIds, $fromDt, $enDt)
    {
		...
		$g_mcc = preg_replace("/^(\d{3})(\d{3})(\d{4})$/", "$1-$2-$3", $_SESSION['g_mcc']); 
		$oAuth2Credential = (new OAuth2TokenBuilder())->fromFile()->withRefreshToken($_SESSION['g_refresh_token'])->build();
...
ParallelReportDownload::main($accIds, $fromDt, $enDt);
