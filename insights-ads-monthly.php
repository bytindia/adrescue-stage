...
function getWeekMonSun($weekOffset) {
	...
	$dates_array[0] = strtotime("first day of last month");
	$dates_array[1] = strtotime("last day of last month");
...
//d($weeks); exit;
									...
									foreach($getData as $key => $getD) 
									{
										...
										for($y=0;$y<1;$y++) 
										{
											...
											$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, 'Month : '.date("d", $weeks[$y][0]).' to '.date("d F", $weeks[$y][1]))->getStyle('A'.$rowCount)->applyFromArray($style);
										...
										$fileN = str_replace(" ","_", $getData[$key]['client_name']).'_Ads-Monthly-Report_'.date('d-M-Y').'.xlsx';
