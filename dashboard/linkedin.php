...
$pg='linkedin';
include 'config.php';
$pgName = $_SESSION['client_name'].' : LinkedIn Ads';
?>
...
foreach($in_acc_ids as $k => $val) {
    if($filter=='no') {
        ...
        $gResq = LinkedInAPI($val,$linkedIn,'0 days','0 days');
        $resData[$val][1] = array($gResq['cost'], $gResq['conversions'], @($gResq['conversions']/$gResq['cost']));
        
        $gResq2 = LinkedInAPI($val,$linkedIn,'-1 days','-1 days');
...
foreach($in_acc_ids as $k => $val) {
    ...
    ?>
        <tr>
            <td><?php echo $i; ?></td>
            <td class="text-left"><?php echo $inAccN[$val]; ?></td>
            <?php if($filter=='no') { ?>
...
?>
        <tr class="tot_row">
            ...
            <td><b><?php if($lead_t_tot>0 && $lead_t_tot!='-') { echo moneyFormatIndia(@($spend_t_tot/$lead_t_tot)); } else { echo '-'; } ?></b></td> 
            
            <td><b><?php echo moneyFormatIndia($spend_y_tot); ?></b></td>
            <td><b><?php echo moneyFormatIndia($lead_y_tot); ?></b></td>
            <td><b><?php if($lead_y_tot>0 && $lead_y_tot!='-') { echo moneyFormatIndia(@($spend_y_tot/$lead_y_tot)); } else { echo '-'; } ?></b></td>
            <?php } ?>
            <td><b><?php echo moneyFormatIndia($spend_30_tot); ?></b></td>
            <td><b><?php echo moneyFormatIndia($lead_30_tot); ?></b></td>
            <td><b><?php if($lead_30_tot>0 && $lead_30_tot!='-') { echo moneyFormatIndia(@($spend_30_tot/$lead_30_tot)); } else { echo '-'; } ?></b></td>
        </tr>
</table>
