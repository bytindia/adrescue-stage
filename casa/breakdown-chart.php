...
if(isset($_SESSION['st'])) { 
 ...
 //$ad_acc = array(569001103516290=>'Aria');
  ...
  $color_arr = array('primary','danger','success','primary','warning','info','danger','success','danger','info','primary','danger','success','danger','warning');$i=0;
    ...
    { //shuffle($color_arr); ?>
    <a class="btn btn-sm btn-<?php echo $color_arr[$i]; ?>" href="loading.php?pg=breakdown-chart.php&act_id=<?php echo $k; ?>"><?php echo $v; ?></a>
    <?php $i++; } ?>
...
Highcharts.chart('container_<?php echo $v; ?>', {
    chart: {type: 'column'},
    title: {text: '<?php echo $v; ?> - Hourly breakdown'},
    subtitle: {text: 'Project: <?php echo $ad_acc[$acc_id]; ?>'},
...
$('#reportrange span').html(d1 + ' - ' + d2);
        ...
        function(start, end) {
            if(start.format('DD/MM/YYYY')==defSt) {
                ...
                $('#stDt_upd').val('');
                $('#enDt_upd').val('');
                window.location = 'loading.php?pg=breakdown-chart.php&act_id=<?php echo $acc_id; ?>';
            
            } else {
                //alert(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                $('#reportrange span').html(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
                window.location = 'loading.php?pg=breakdown-chart.php&act_id=<?php echo $acc_id; ?>&st='+start.format('DD/MM/YYYY')+'&en='+end.format('DD/MM/YYYY');
                startDate = start;
                endDate = end;   
